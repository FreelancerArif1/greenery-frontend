...
const MyComponent = ({ data,sectionData }) => {
  ...
  return (
    <StyledComponent offset={offset} className={`product-sliders pt-120`}>
      <Container>
        <Row className="product-sliders__top">
          <Col md={10}>
            <Title text={sectionData?.section_data?.subtitle} margin={'0 0 20px 0'}/>
            <p className={'fade-up'}>{reactHtmlParser(sectionData?.section_data?.short_desc)}</p>
          </Col>
          <Col md={2}>
      ...
      <Container fluid className={"p-0"}>
        {data?.length > 0 && (
          <Swiper
            ...
            speed={900}
            navigation={{
              prevEl: `.product-sliders #service-prev`,
              nextEl: `.product-sliders #service-next`,
            }}
          ...
          >
            {data.sort((a, b) => (a?.product_data?.sort_order || 0) - (b?.product_data?.sort_order || 0)).map((p, i) => (
              <SwiperSlide key={(p?.product_data?.id ?? i)}>
                <Link href={`/products/${p?.product_data?.slug}`}>
